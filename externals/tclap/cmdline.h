...
class CmdLine : public CmdLineInterface
{
	protected:
		...
		/**
		 * Checks whether a name/flag wstring matches entirely matches
	...
	public:
		...
		CmdLine(const std::wstring& message,
				...
				const std::wstring& version = _T("none"),
		 ...
		 */
		void parse(int argc, TCHAR ** argv, bool bExit);
...
inline void CmdLine::_constructor()
{
	...
	if ( _helpAndVersion )
	{
		...
		SwitchArg* help = new SwitchArg( _T("h"), _T("help"),
						_T("Displays usage information and exits."),
						false, v);
		add( help );
		deleteOnExit(help);
		deleteOnExit(v);
		...
		SwitchArg* vers = new SwitchArg( _T(""), _T("version"),
					_T("Displays version information and exits."),
					false, v);
		add( vers );
		deleteOnExit(vers);
		deleteOnExit(v);
	...
	SwitchArg* ignore  = new SwitchArg(Arg::flagStartString(),
			   ...
			   _T("Ignores the rest of the labeled arguments following this flag."),
					   false, v);
	add( ignore );
	deleteOnExit(ignore);
	deleteOnExit(v);
...
inline void CmdLine::xorAdd( std::vector<Arg*>& ors )
{
	...
	{
		(*it)->forceRequired();
		(*it)->setRequireLabel( _T("OR required") );
	...
	for( ArgListIterator it = _argList.begin(); it != _argList.end(); it++ )
		if ( *a == *(*it) )
			throw( SpecificationException(
			       	_T("Argument with same flag/name already exists!"),
					a->longID() ) );
...
inline void CmdLine::parse(int argc, TCHAR ** argv, bool bExit)
{
	try {
  	...
  	for (int i = 0; static_cast<unsigned int>(i) < args.size(); i++)
	{
		...
		if ( !matched && !Arg::ignoreRest() )
			throw(CmdLineParseException( _T("Couldn't find match for argument"),
			                             args[i]));
    }

	if ( requiredCount < _numRequired )
		throw(CmdLineParseException( _T("One or more required arguments missing!") ));

	if ( requiredCount > _numRequired )
		throw(CmdLineParseException( _T("Too many arguments!") ));
	...
	catch ( ArgException e )
	{
		...
		{
			exit(1); 
			std::wcout << _T("Exiting program.") << std::endl;
		}
	}
}
