 ...
 */
template<>
class ValueExtractor<std::wstring> 
{
	friend class MultiArg<std::wstring>;
   	...
   	private:
		 ...
		 */
        ValueExtractor(std::vector<std::wstring> &values) : _values(values) {}
...
class MultiArg : public Arg
{
	protected:
		 ...
		 * is thrown.
		 * \param val - The wstring to be read.
		 */
	...
	public:
		...
		/**
		 * Returns the a short id wstring.  Used in the usage. 
		 * \param val - value to be used.
		 */
		virtual std::wstring shortID(const std::wstring& val="val") const;

		/**
		 * Returns the a long id wstring.  Used in the usage. 
		 * \param val - value to be used.
		 */
...
bool MultiArg<T>::processArg(int *i, std::vector<std::wstring>& args) 
{
	...
	std::wstring value = _T("");
   	...
   	if ( argMatches( flag ) )
   	{
		...
		// always take the first one, regardless of start wstring
		if ( value == _T("") )
		{
			(*i)++;
		...
		/*
		// continuing taking the args until we hit one with a start wstring 
...
{
	std::wstring id = Arg::shortID(_typeDesc) + _T(" ... ");
...
{
	std::wstring id = Arg::longID(_typeDesc) + _T("  (accepted multiple times)");
...
void MultiArg<T>::_extractValue( const std::wstring& val ) 
{
	...
	if ( err == MULTI_ARG_HELPER::EXTRACT_FAILURE )
		throw( ArgParseException(_T("Couldn't read argument value ")
                                 _T("from wstring '") + val + _T("'"), toString() ) );

	if(err == MULTI_ARG_HELPER::EXTRACT_TOO_MANY)
	    throw( ArgParseException(_T("More than one valid value ")
                                 _T("parsed from wstring '") + val + _T("'"), 
								 toString() ) );		    
	if ( _constraint != NULL )
