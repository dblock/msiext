...
UnlabeledMultiArg<T>::UnlabeledMultiArg(const std::wstring& name, 
										...
										bool ignoreable,
					                    Visitor* v)
: MultiArg<T>(_T(""), name, desc,  req, typeDesc, v)
{ 
...
UnlabeledMultiArg<T>::UnlabeledMultiArg(const std::wstring& name, 
										...
										bool ignoreable,
					                    Visitor* v)
: MultiArg<T>(_T(""), name, desc,  req, constraint, v)
{ 
	_ignoreable = ignoreable;
	OptionalUnlabeledTracker::check(true, toString());
}
...
bool UnlabeledMultiArg<T>::processArg(int *i, std::vector<std::wstring>& args) 
{
	...
	// always take the first value, regardless of the start wstring 
	_extractValue( args[(*i)] );

	/*
	// continue taking args until we hit the end or a start wstring 
...
{
	std::wstring id = _T("<") + _typeDesc + _T("> ...");
...
{
	std::wstring id = _T("<") + _typeDesc + _T(">  (accepted multiple times)");
